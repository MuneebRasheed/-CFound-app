...
export default function Found({ navigation, route }) {
  ...
  useLayoutEffect(() => {
    const collectionRef = collection(db, 'Found');
  ...
  async function Get() {
    let arr = [];
    const q = query(collection(db, "Found"), where('Category', "==", route.params.title));
    ...
    <View style={styles.container}>
      ...
      <View style={{ flex: 0.89 }}>
        ...
        <TouchableOpacity style={styles.addIcon}
          onPress={() => navigation.navigate('FoundItemAdd', { title: route.params.title })}>
